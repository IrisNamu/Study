...
public class ViewController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 게시물 불러오기
		MVCBoardDAO dao = new MVCBoardDAO(); // 다오 객체 생성
		String idx = request.getParameter("idx"); // 게시물의 일련번호를 매개변수로 받아
		dao.updateVisitCount(idx); // 조회수를 1 증가
		...
		request.getRequestDispatcher("/14MVCBoard/View.jsp").forward(request, response);//View.jsp로 포워드
	}
